...
angular.module('hisApp')
        .controller('TitleCtrl', function($scope, TitleService) {
            $scope.search = "";
            $scope.oldSearch = "";
            $scope.paging = {
                currentPage: 1,
...
//                $scope.paging.currentPage = $scope.search != $scope.oldSearch ? 1 : $scope.currentPage;
                $scope.dataPage = TitleService.query($scope.search, $scope.paging.currentPage, function() {
...
//            $scope.$watch('paging.currentPage', $scope.reloadData, true);
            $scope.edit = function(x) {
                ...
                if (x.id) {
                    TitleService.get(x.id).success(function(data){
            ...
            $scope.remove = function(x) {
                bootbox.confirm('Anda yakin untuk mengahapus kategori[' + x.nama + '] ?', function(result) {
                    if (result) {
                        TitleService.remove(x).success(function() {
                            $scope.clear();
                        });
                    }
                });
            ...
            $scope.save = function() {
                TitleService.findByNama($scope.currentRecord.nama).success(function(data) {
                    if (data && (data.id !== $scope.currentRecord.id)) {
                        $scope.isNameExists = true;
                        return;
                    } else {
                        TitleService.save($scope.currentRecord).success(function() {
