...
angular.module('hisApp')
        .controller('HrProfileCtrl', function($scope, HrProfileService) {
            $scope.search = "";
            $scope.oldSearch = "";
            $scope.paging = {
                currentPage: 1,
...
//                $scope.paging.currentPage = $scope.search != $scope.oldSearch ? 1 : $scope.currentPage;
                $scope.dataPage = HrProfileService.query($scope.search, $scope.paging.currentPage, function() {
...
//            $scope.$watch('paging.currentPage', $scope.reloadData, true);
            $scope.edit = function(x) {
                ...
                if (x.id) {
                    HrProfileService.get(x.id).success(function(data){
            ...
            $scope.remove = function(x) {
                bootbox.confirm('Anda yakin untuk mengahapus HR Profile [' + x.nama + '] ?', function(result) {
                    if (result) {
                        HrProfileService.remove(x).success(function() {
                            $scope.clear();
                        });
                    }
                });
            ...
            $scope.save = function() {
                HrProfileService.findByNama($scope.currentRecord.nama).success(function(data) {
                    if (data && (data.id !== $scope.currentRecord.id)) {
                        $scope.isNameExists = true;
                        return;
                    } else {
                        HrProfileService.save($scope.currentRecord).success(function() {
